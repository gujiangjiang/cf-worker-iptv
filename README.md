...
- **安全鉴权**：后台管理页面受密码保护。
- **模块化设计**：前端与后端逻辑分离，后端进一步按功能拆分为多模块，易于维护。

---

## 🚀 快速部署

### 方式一：使用 Cloudflare 网页后台 (适合简单操作)
...
3. **上传代码**
   - 点击 **Edit code**。
   - 推荐使用 CLI 方式，因为本项目包含多个文件。
   - 如果必须使用网页端，请按照本项目的目录结构 (`src/js/`, `src/front/`) 手动创建文件并粘贴对应的代码。入口文件 (`main`) 请设置为 `src/js/worker.js`。
